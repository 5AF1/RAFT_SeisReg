...
class SeismicRAFT(nn.Module):
    def __init__(self, args):
        ...
        if args.small:
            self.fnet = SeismicSmallEncoder(output_dim=128, norm_fn='instance', dropout=args.dropout)        
            self.cnet = SeismicSmallEncoder(output_dim=hdim+cdim, norm_fn='none', dropout=args.dropout)
            self.update_block = SeismicSmallUpdateBlock(self.args, hidden_dim=hdim)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, upsample=True, test_mode=False):
        ...
        if self.args.equalize:
            image1 = 2 * (image1 / 1023.0) - 1.0
            image2 = 2 * (image2 / 1023.0) - 1.0
